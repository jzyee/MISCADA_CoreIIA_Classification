...
rec_score <- yardstick::recall_vec(as.factor(epil_test_y),
                                   as.factor(pred_test_res))

barplot(c(rec_score,0.941,0.4),
        names.arg =c('MLP','SVM','baseline'),
        col=c('red','blue', 'green'),
        main='recall comparison')
abline(h=0.941)



acc_score <- yardstick::accuracy_vec(as.factor(epil_test_y),
                                   as.factor(pred_test_res))

barplot(c(acc_score,0.955,0.49),
        names.arg =c('MLP','SVM','baseline'),
        col=c('red','blue','green'),
